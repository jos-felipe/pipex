...
((TEST_NO++))
echo -e $YELLOW_BG"$TEST_NO. Invalid cmd2"$COLOR_LIMITER
valgrind	--leak-check=full \
			--show-reachable=yes \
			--track-fds=yes \
			--show-leak-kinds=all -s \
			--track-origins=yes \
			--log-file=$VALGRIND_LOG \
$NAME $INFILE "cat" "" $OUTFILE > /dev/null 2>&1
cat $VALGRIND_LOG | grep "All heap blocks were freed -- no leaks are possible" > /dev/null
if [ $? -eq 0 ]; then
	echo -e $GREEN"[OK]"$COLOR_LIMITER;
else
...
((TEST_NO++))
echo -e $YELLOW_BG"$TEST_NO. Space instead of cmd1"$COLOR_LIMITER
valgrind	--leak-check=full \
			--show-reachable=yes \
			--track-fds=yes \
			--show-leak-kinds=all -s \
...
((TEST_NO++))
echo -e $YELLOW_BG"$TEST_NO. Space instead of cmd2"$COLOR_LIMITER
valgrind	--leak-check=full \
			--show-reachable=yes \
			--track-fds=yes \
			--show-leak-kinds=all -s \
			--track-origins=yes \
			--log-file=$VALGRIND_LOG \
$NAME $INFILE "echo Hello" " " $OUTFILE > /dev/null 2>&1
cat $VALGRIND_LOG | grep "All heap blocks were freed -- no leaks are possible" > /dev/null
if [ $? -eq 0 ]; then
	echo -e $GREEN"[OK]"$COLOR_LIMITER;
else
